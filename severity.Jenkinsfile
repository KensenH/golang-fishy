		...
		stage('Version') {
			steps {
				script {
					...
					sh "yq -i e '.container.image = \"${DOCKER_IMAGE_URL}:severity${env.VERSION}\"' charts/values.yaml"
				}
		...
		stage('Compile and Dockerize') {
			steps {
				script {
                    sh "docker ps"
					echo '> Creating image ...'
					sh "docker build . -t ${DOCKER_IMAGE_URL}:severity${env.VERSION}"
					echo '> Pushing image ...'
                    sh "docker push ${DOCKER_IMAGE_URL}:severity${env.VERSION}"
					sh "docker rmi ${DOCKER_IMAGE_URL}:severity${env.VERSION}"
				}
			}
		}
